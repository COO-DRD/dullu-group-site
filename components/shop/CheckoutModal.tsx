...
export default function CheckoutModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMsg(data.alreadyOwned ? "You already own this — check your email for the link." : (data.error ?? "Checkout failed."));
        setStep("error");
        return;
      }
        ...
        {step === "form" && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="font-sans text-[10px] leading-relaxed pt-2" style={{ color: "#CCCCCC" }}>
              ...
              {!isFree && (
                <a href="/terms" className="underline underline-offset-2" style={{ color: "#AAAAAA" }}>
                  Terms of Service
                </a>
              )}
              {!isFree && "."}
        ...
        {step === "success" && (
          <div className="py-4">
            <div className="text-center">
              <div
                className="w-12 h-12 flex items-center justify-center mx-auto mb-6 rotate-45"
